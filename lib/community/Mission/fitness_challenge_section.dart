...
class FitnessChallengeSection extends StatefulWidget {
  const FitnessChallengeSection({super.key});

  @override
  _FitnessChallengeSectionState createState() =>
      _FitnessChallengeSectionState();
}

class _FitnessChallengeSectionState extends State<FitnessChallengeSection> {
  List<Post> posts = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> _fetchPosts() async {
    try {
      List<Post> fetchedPosts = await ApiService().getPosts(7);
      setState(() {
        posts = fetchedPosts;
        isLoading = false;
      });
