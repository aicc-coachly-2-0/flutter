...
class DietPostSection extends StatefulWidget {
  const DietPostSection({super.key});

  @override
  _DietPostSectionState createState() => _DietPostSectionState();
}

class _DietPostSectionState extends State<DietPostSection> {
  List<Post> posts = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> _fetchPosts() async {
    try {
      List<Post> fetchedPosts = await ApiService().getPosts(6);
      setState(() {
        posts = fetchedPosts;
        isLoading = false;
      });
