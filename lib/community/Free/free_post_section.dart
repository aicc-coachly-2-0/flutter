...
class FreePostListSection extends StatefulWidget {
  const FreePostListSection({super.key});

  @override
  _FreePostListSectionState createState() => _FreePostListSectionState();
}

class _FreePostListSectionState extends State<FreePostListSection> {
  List<Post> posts = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> _fetchPosts() async {
    try {
      List<Post> fetchedPosts = await ApiService().getPosts(1);
      setState(() {
        posts = fetchedPosts;
        isLoading = false;
      });
