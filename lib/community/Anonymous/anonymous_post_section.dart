...
import 'package:flutter_application_test/community/Anonymous/anonymous_post.dart';
...
class AnonymousPostSection extends StatefulWidget {
  const AnonymousPostSection({super.key});

  @override
  _AnonymousPostSectionState createState() => _AnonymousPostSectionState();
}

class _AnonymousPostSectionState extends State<AnonymousPostSection> {
  List<Post> posts = [];
  bool isLoading = true;
  String errorMessage = '';
  ...
  Future<void> _fetchPosts() async {
    try {
      List<Post> fetchedPosts = await ApiService().getPosts(2);
      setState(() {
        posts = fetchedPosts;
        isLoading = false;
      });
              ...
              child: ListTile(
                ...
                onTap: () {
                  ...
                  Navigator.push(
                    ...
                    MaterialPageRoute(
                      builder: (context) =>
                          AnonymousPost(post: post), // UserPost로 데이터 전달
                    ),
                  );
                },
              ),
